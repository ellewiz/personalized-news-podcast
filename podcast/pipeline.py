...
CATEGORY_PREFERENCES = {
    ...
    "markets": (
        "Cover ONLY the 2 biggest stories, 3 at most — do not try to cover everything even if "
        "there's a lot of market news. Pick the single most significant items and skip the "
        "rest entirely, no matter how much is in the list below. This airs before the US "
        "market opens — name which session/market each item refers to inside the sentence "
        "reporting it (e.g. an Asian market's session ended hours ago and is not concurrent "
        "with a US session that hasn't started yet); never imply everything is happening "
        "'today' in the same sense. Always state the currency or unit for every number "
        "explicitly (e.g. '16 rand per dollar,' not '16 per dollar') — never leave a bare "
        "number without its unit. If there's a major acquisition, merger, or leveraged buyout "
        "(LBO) announced, cover it even if it would otherwise be bumped for space — name who's "
        "involved (acquirer and target), the deal size if reported, and the anticipated close "
        "date or timeline if mentioned in the source material. At the 6am ET broadcast time: "
        "Asian markets (Tokyo, Hong Kong, Shanghai) have already closed for the day; European "
        "markets (London, Frankfurt, Paris) are open and mid-session; the US market hasn't "
        "opened yet. Never describe European markets as 'already wrapped' or 'closed' at "
        "broadcast time — only Asia has actually closed by then. Use these facts to keep "
        "specific claims accurate (e.g. 'which sent shares higher after Wednesday's close') — "
        "do NOT turn them into a standalone reminder sentence to the listener. Never write a "
        "sentence starting 'Remember,' 'Keep in mind,' or 'As a reminder' about market timing, "
        "and never state the same timing caveat more than once in this segment. She listens at "
        "6am and already knows the US market is shut, so never tell her the US market or "
        "session hasn't opened yet — not as an opening clause, not as a trailing one, not in "
        "any rewording. Everything that caveat is trying to convey is already conveyed by "
        "naming where a move actually happened: say 'futures are pointing lower,' 'in "
        "premarket trading,' 'in Wednesday's session,' or 'after Thursday's close' inside the "
        "claim itself, and then say nothing further about the clock. If a company's news or "
        "earnings releases outside trading hours (after close or before open), say the "
        "reaction is showing up in after-hours, pre-market, or futures trading — don't imply "
        "it happened in a session that had already ended before the news broke."
    ),
    "nfl": (
...
def run() -> Path:
    ...
    # Each segment is generated by its own stateless API call with no visibility into
    # the others, so left alone they tend to independently converge on the same
    # generic opener ("Turning to X..."). Passing forward the opening sentences
    # already used this episode gives each subsequent segment enough awareness to
    # vary its own, without a separate full-episode editing pass.
    ...
    for category in CATEGORY_ORDER:
        ...
        try:
            ...
            if items:
                used_openers.append(script.first_sentence(segment.text))
        except Exception as exc:
